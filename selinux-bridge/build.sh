...
# Stamp the build time into the code so that "bridge_client info" can name
# the build that is actually answering. An in-place update does not reliably
# restart the service, and a bridge still running the previous build is
# otherwise indistinguishable from one that updated cleanly (see "Stale
# process after an update" in README.md).
mkdir -p build/gen/com/selinuxbridge/app
cat > build/gen/com/selinuxbridge/app/BuildStamp.java <<EOF
package com.selinuxbridge.app;

/** Generated by build.sh -- do not edit. */
public final class BuildStamp {
    public static final long BUILD_TIME = ${SOURCE_DATE_EPOCH:-$(date +%s)}000L;
    private BuildStamp() {}
}
EOF
javac --release 8 -d build/classes -classpath "$PLATFORM_JAR" \
  app/src/main/java/com/selinuxbridge/app/*.java \
  build/gen/com/selinuxbridge/app/BuildStamp.java \
  build/gen/com/selinuxbridge/app/R.java
